...
%     view(180,0)
    ...
    ylabel('Range Sample')
    zlabel('Magnitude [dB]');
     title({'H left';['Noise floor = ' num2str(noise_floor_emp)  ' [dB].  Signal strength = ' num2str(signal_strength_emp)  ' [dB]'];['SNR = ' num2str(signal_strength_emp - noise_floor_emp)  '[dB]']})
% title({ [num2str(mean(real(noise_floor_mat(:))))] ; [num2str(mean(imag(noise_floor_mat(:))))]; [num2str(var(real(noise_floor_mat(:))))]});  
...
%     view(180,0)
    ...
    ylabel('Range Sample')
    zlabel('Magnitude [dB]');
     title({'H right';['Noise floor = ' num2str(noise_floor_emp)  ' [dB].  Signal strength = ' num2str(signal_strength_emp)  ' [dB]'];['SNR = ' num2str(signal_strength_emp - noise_floor_emp)  '[dB]']})
% title({ [num2str(mean(real(noise_floor_mat(:))))] ; [num2str(mean(imag(noise_floor_mat(:))))]; [num2str(var(real(noise_floor_mat(:))))]});  
axis([-inf inf -inf inf -90 10 ])
...
%     view(180,0)
    ...
    ylabel('Range Sample')
    zlabel('Magnitude [dB]');
    title({'V left';['Noise floor = ' num2str(noise_floor_emp)  ' [dB].  Signal strength = ' num2str(signal_strength_emp)  ' [dB]'];['SNR = ' num2str(signal_strength_emp - noise_floor_emp)  '[dB]']})
%  title({ [num2str(mean(real(noise_floor_mat(:))))] ; [num2str(mean(imag(noise_floor_mat(:))))]; [num2str(var(real(noise_floor_mat(:))))]});   
 axis([-inf inf -inf inf -90 10 ])
...
%     view(180,0)
    ...
    ylabel('Range Sample')
    zlabel('Magnitude [dB]');
     title({'V right';['Noise floor = ' num2str(noise_floor_emp)  ' [dB].  Signal strength = ' num2str(signal_strength_emp)  ' [dB]'];['SNR = ' num2str(signal_strength_emp - noise_floor_emp)  '[dB]']});
%     title({ [num2str(mean(real(noise_floor_mat(:))))] ; [num2str(mean(imag(noise_floor_mat(:))))]; [num2str(var(real(noise_floor_mat(:))))]});
     axis([-inf inf -inf inf -90 10 ])
    eval(['cd ' ResultsDir]);
...
subplot(221)
plot(t,real(TGT_H_left_mf_IQ(:,257))*32768,t,imag(TGT_H_left_mf_IQ(:,257))*32768); 
grid on;
xlabel('Range Sample');
ylabel('Amplitude')
title('H left')
...
subplot(222)
plot(t,real(TGT_H_right_mf_IQ(:,257))*32768,t,imag(TGT_H_right_mf_IQ(:,257))*32768); 
grid on;
xlabel('Range Sample');
ylabel('Amplitude')
title('H right')
...
subplot(223)
plot(t,real(TGT_V_left_mf_IQ(:,257))*32768,t,imag(TGT_V_left_mf_IQ(:,257))*32768); 
...
subplot(224)
plot(t,real(TGT_V_right_mf_IQ(:,257))*32768,t,imag(TGT_V_right_mf_IQ(:,257))*32768); 
...
%     'LineStyle','none');
    ...
    subplot(1,4,1)
    [sample,buffer_num]=my_max_matrix(20*log10(abs(TGT_H_left_mf)));
   bar(sample(1) - width:sample(1) + width,20*log10(abs(TGT_H_left_mf(sample(1) - width:sample(1) + width,buffer_num))),'BaseValue',-100)
...
%     text(2, -width + 1 ,7, ['Max Range:' num2str(sample(1))]);
 ...
 subplot(1,4,2)
     [sample,buffer_num]=my_max_matrix(20*log10(abs(TGT_H_right_mf)));
       bar(sample(1) - width:sample(1) + width,20*log10(abs(TGT_H_right_mf(sample(1) - width:sample(1) + width,buffer_num))),'BaseValue',-100)
...
%     text(2, -width + 1 ,7, ['Max Range:' num2str(sample(1))]);
  ...
  subplot(1,4,3)
     [sample,buffer_num]=my_max_matrix(20*log10(abs(TGT_V_left_mf)));
      bar(sample(1) - width:sample(1) + width,20*log10(abs(TGT_V_left_mf(sample(1) - width:sample(1) + width,buffer_num))),'BaseValue',-100)
...
%     text(2, -width + 1 ,7, ['Max Range:' num2str(sample(1))]);
  ...
  subplot(1,4,4)
   [sample,buffer_num]=my_max_matrix(20*log10(abs(TGT_V_right_mf)));
   bar(sample(1) - width:sample(1) + width,20*log10(abs(TGT_V_right_mf(sample(1) - width:sample(1) + width,buffer_num))),'BaseValue',-100)
...
%     text(2, -width + 1 ,7, ['Max Range:' num2str(sample(1))]);
    ...
    subplot(1,4,1)
    [sample,buffer_num]=my_max_matrix(20*log10(abs(TGT_H_left_mf)));
     bar(width:-1:-width,20*log10(abs(TGT_H_left_mf(sample(1),buffer_num - width : buffer_num + width))),'BaseValue',-100)
%     mesh(width:-1:-width,-width:width,20*log10(TGT_H_left_mf(sample(1) - width: sample(1) + width,buffer_num(1) - width:buffer_num(1) + width)))
    hold on;
    grid on;
%    plot3(ones(1,length(t)),t,20*log10(TGT_H_left_mf_mean)+40,'linewidth',2,'Color','k')
...
%     text(2, -width + 1 ,7, ['Max Range:' num2str(sample(1))]);
 ...
 subplot(1,4,2)
     [sample,buffer_num]=my_max_matrix(20*log10(abs(TGT_H_right_mf)));
          bar(width:-1:-width,20*log10(abs(TGT_H_right_mf(sample(1),buffer_num - width : buffer_num + width))),'BaseValue',-100)
%    mesh(width:-1:-width,-width:width,20*log10(TGT_H_right_mf(sample(1) - width: sample(1) + width,buffer_num(1) - width:buffer_num(1) + width)))
    hold on;
    grid on;
%     plot3(ones(1,length(t)),t,20*log10(TGT_H_right_mf_mean)+40,'linewidth',2,'Color','k')
...
%     text(2, -width + 1 ,7, ['Max Range:' num2str(sample(1))]);
  ...
  subplot(1,4,3)
     [sample,buffer_num]=my_max_matrix(20*log10(abs(TGT_V_left_mf)));
        bar(width:-1:-width,20*log10(abs(TGT_V_left_mf(sample(1),buffer_num - width : buffer_num + width))),'BaseValue',-100)
%      mesh(width:-1:-width,-width:width,20*log10(TGT_V_left_mf(sample(1) - width: sample(1) + width,buffer_num(1) - width:buffer_num(1) + width)))
    hold on;
    grid on;
%     plot3(ones(1,length(t)),t,20*log10(TGT_V_left_mf_mean)+40,'linewidth',2,'Color','k')
...
%     text(2, -width + 1 ,7, ['Max Range:' num2str(sample(1))]);
  ...
  subplot(1,4,4)
   [sample,buffer_num]=my_max_matrix(20*log10(abs(TGT_V_right_mf)));
           bar(width:-1:-width,20*log10(abs(TGT_V_right_mf(sample(1),buffer_num - width : buffer_num + width))),'BaseValue',-100)
%     mesh(width:-1:-width,-width:width,20*log10(TGT_V_right_mf(sample(1) - width: sample(1) + width,buffer_num(1) - width:buffer_num(1) + width)))
    hold on;
    grid on;
%     plot3(ones(1,length(t)),t,20*log10(TGT_V_right_mf_mean)+40,'linewidth',2,'Color','k')
