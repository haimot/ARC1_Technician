...
namespace ARC1_Technician
{
	...
	void  MyForm::B_LOAD_MAP_Click(System::Object^  sender, System::EventArgs^  e)
	{
		...
		//--------------------------------------
		for_debug_timer_7 = clock();
		if (Radar_Viewer.Load_TOM_From_File() == false)
		{
			My_Message("TOM file is incorrect.");
			return;
		}	
		for_debug_timer_8 = clock();
		//--------------------------------------
		//	Plot to output for debuging
		//--------------------------------------
		wchar_t text_buffer[200] = { 0 }; //temporary buffer
		swprintf(text_buffer, _countof(text_buffer), L"'Load TOM' plot Elapsed time: 7 = %f \n", double(for_debug_timer_8 - for_debug_timer_7) / CLOCKS_PER_SEC); // convert
	...
	//  Return Value:	GOOD/FAULT
	// --------------------------------------------------
	int MyForm::Process_obstacles_map(char* pBuffer)
	{
		double				work_TX_mode;
		...
		p_targets += sizeof(uint32_t);
		Targets_authentication_end = (uint32_t*)(p_targets + sizeof(T_TARGETS_data));
		//-----------------------------------------------
		//					Trajectories 
		//-----------------------------------------------
		Point_To_Trajectories = (uint32_t*)((uint8_t*)Targets_authentication_end + 2 * sizeof(uint32_t) + decimated_range_samples * System_Status->system_control_0->SYSTEM_control_shadow.DET_algorithms.Maximal_doppler * 2 * 4 + sizeof(uint32_t));
	...
	void MyForm::timer_DSP_messages_Tick(System::Object^  sender, System::EventArgs^  e)
	{
		...
		if ((System_Status->Monitor_Operation_mode == OPERATION_WORK))
		{
			if (System_Status->system_control_0->DATA_download.PC_data_authentication != local_authentication)
			{
				...
				//-------------------------------------------
				decimated_range_samples = System_Status->system_control_0->SYSTEM_control_shadow.DET_algorithms.RANGE_samples / System_Status->system_control_0->SYSTEM_control_shadow.DET_algorithms.Range_decimation;
				buffer_size_char = sizeof(T_TARGETS_data) + sizeof(double) + decimated_range_samples * System_Status->system_control_0->SYSTEM_control_shadow.DET_algorithms.Maximal_doppler * 2 * sizeof(short) * 2 * 2 * 2 + sizeof(uint32_t) * (3 + 3) + 2 * sizeof(uint32_t);
	...
	//  For QA
	//------------------------------------
	void MyForm::B_Draw_Triangle_Click(System::Object^  sender, System::EventArgs^  e)
	{
		Graphics^ g = B_MAP_PANEL->CreateGraphics();

		// Create a GraphicsPath object.
		GraphicsPath^ myPath = gcnew GraphicsPath;

		// Create a GraphicsPath object.
		GraphicsPath^ my_Triangle_Path = gcnew GraphicsPath;


		Point	p1				= Point(100, 100);
		Point	p2				= Point(300, 130);
		Point   p3				= Point(120, 100);
		Point   p4				= Point(110, 85);
		PointF	rotate_Point	= p2;
		myPath->AddLine(p1, p2);

		array<Point>^ triangle_points = { p1, p3, p4 };
		my_Triangle_Path->AddPolygon(triangle_points);
		...
		// Create solid brush.
		SolidBrush^ yellow_Brush = gcnew SolidBrush(Color::Yellow);

		//g->DrawPath(pen, my_Triangle_Path);

		// Fill graphics path to screen.
		g->FillPath(yellow_Brush, my_Triangle_Path);

		// Create a matrix and rotate it 30 degrees.
		Matrix^ rotate_Matrix = gcnew Matrix;
		rotate_Matrix->RotateAt(30, rotate_Point, MatrixOrder::Append);

		// Draw the rectangle to the screen again after applying the
		// transform.
		//g->Transform = rotate_Matrix;
		//g->DrawPath(pen, myPath);

		// Create a matrix and rotate it 30 degrees.
		//Matrix^ rotate_Matrix = gcnew Matrix;
		//delete rotate_Matrix;
		///rotate_Matrix = gcnew Matrix;
		//rotate_Matrix->RotateAt(60, rotate_Point, MatrixOrder::Append);
		...
		//Matrix^ translate_Matrix = gcnew Matrix;
		//translate_Matrix->Translate(100, 250);
		//myPath->Transform(translate_Matrix);
