...
namespace ARC1_Technician
{
	...
	void Recovery_Flight::B_PLAY_RE_FLIGHT_VIEWER_Click(System::Object^  sender, System::EventArgs^  e)
	{
		...
		if (operation_mode != 0)
		{
			...
			{
				Radar_Viewer.Restart_Offline();
			}

			operation_mode = 0;
			flag = true;

			Path = "images\\Media_Controls_Pause_icon_32_pixel.png";
		...
		else if (operation_mode == 0)
		{
			...
			//--------------------------------
			operation_mode = 2;
			Path = "images\\Media_Controls_Play_icon_32_pixel.png";
			image = gcnew Bitmap(Path, true);
			B_PLAY_RE_FLIGHT_VIEWER->Image = image;
		}
	}
	...
	void Recovery_Flight::B_MAP_PANEL_RE_FLIGHT_Paint(System::Object^  sender, System::Windows::Forms::PaintEventArgs^  e)
	{
		...
		//Graphics^ g = B_MAP_PANEL_RE_FLIGHT->CreateGraphics();
		...
		//myMatrix->Scale(3, 3, MatrixOrder::Append);

		////// Draw the rectangle to the screen again after applying the
		////// transform.
		//g->Transform = myMatrix;
		//g->DrawRectangle(myPen2, 50, 50, 100, 100);

		//delete myMatrix;
		//myMatrix = gcnew Matrix;
		//myMatrix->Shear(2, 0);
		//g->DrawRectangle(myPen, 50, 50, 100, 100);
		//g->DrawRectangle(gcnew Pen(Color::Green), 0, 0, 100, 50);
		//g->MultiplyTransform(myMatrix);
		//g->DrawRectangle(gcnew Pen(Color::Red), 0, 0, 100, 50);
		//g->DrawEllipse(gcnew Pen(Color::Blue), 0, 0, 100, 50);
		...
		//Point p = B_MAP_PANEL_RE_FLIGHT->AutoScrollPosition;
		//int x = p.X;
	...
	void Recovery_Flight::B_MAP_PANEL_RE_FLIGHT_MouseWheel(System::Object^ sender, System::Windows::Forms::MouseEventArgs^ e)
	{
		...
		Radar_Viewer.Set_Scale(screen_width_meter);
		// --------------------------------------------------
		// Refresh screen with new scale
		// --------------------------------------------------
		B_MAP_PANEL_RE_FLIGHT->Invalidate();
	...
	{
		if ((B_SCREEN_WIDTH_OF_VIEWER->Value > B_SCREEN_WIDTH_OF_VIEWER->Minimum) && (B_SCREEN_WIDTH_OF_VIEWER->Value < B_SCREEN_WIDTH_OF_VIEWER->Maximum))
		{
			// --------------------------------------------------
			// Calculate the new scale
			// --------------------------------------------------		
	...
	void Recovery_Flight::Operations_Timer_Tick(System::Object^  sender, System::EventArgs^  e)
	{
		if (operation_mode != -1)
		{
			if (returned_message != L"done" && operation_mode != 4) // operation_mode = 4 - 'Stop' case
			{
				...
				{
					Path = gcnew String("images\\Media_Controls_Play_icon_32_pixel.png");
					image = gcnew Bitmap(Path, true);
					B_PLAY_RE_FLIGHT_VIEWER->Image = image;
			...
			else
			{
				...
				Radar_Viewer.Set_Viewer_Scale((int)(GUI_Status.Screen_Width_Of_Viewer));
				operation_mode = -1;
